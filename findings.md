...
- 重新构建并替换容器后，镜像摘要为 `sha256:e3159876d755379b37de67b3af1145707d16c1e90d603f1471e8eee5e7226486`，容器内包含 Agent `meta=1` 状态查询和独立 `/result` 下载路由。
- 历史完成任务 `mtnnapen1a5cm` 验证：状态接口返回 200、响应体约 130 bytes；结果接口返回 200、响应体约 2.49 MB，证明轮询不会再反复携带完整图片。
- 容器保留 `/etc/gpt-image-playground/api-config.env:ro` 与 `/var/lib/gpt-image-playground` 挂载，Node 异步任务服务正常运行，首页 HTTP 200。
- 结论：本次“异步任务超时”主要是旧镜像未真正部署叠加移动端轮询中断/客户端误判；新版本由服务端任务状态作为唯一真相，前端恢复时继续查询原任务，不以页面生命周期或固定 30 分钟截止判定失败。
...
- 提交 `782ad5f` 已部署到 `jdy`，新镜像摘要为 `sha256:dc0cbb4baab2f1393aed64e8a4082f5113f6a4cd61df15264ab29e79a3f52660`；运行容器已包含 Node 异步任务进程和新 Agent 资源。
- 线上验证未调用真实图像上游：首页 200、Agent 创建路由的空请求校验 400、未知任务查询 404，且服务端配置文件三路 Key 均通过非空检查。验收应由用户强制刷新后发起一次实际 Agent 任务。

## Agent 异步任务超时再定位（2026-09-05）

- 远端任务目录核查显示最近的 Agent 任务均已落盘为 `done`；用户本次任务 `mtnnapen1a5cm` 服务端创建于 `00:29:11Z`，完成于 `00:31:38Z`，实际耗时约 146 秒，并非后台执行超时。
- Nginx 日志显示该任务只收到创建请求和最初两次状态查询，之后手机页面没有继续轮询；这与 iOS Safari 切到后台后挂起页面脚本的行为一致。
- 当前 `src/lib/serverManagedAgentApi.ts` 设置了 30 分钟浏览器截止时间。页面挂起期间 `Date.now()` 跨过截止时间后，恢复执行会直接抛出“服务端 Agent 异步任务超时”，即使服务端任务早已完成。
- `src/store.ts` 的 `isNetworkRecoverableError()` 没有识别中文“超时”，因此该异常会进入普通失败分支，清除 `round.serverTaskId`，使后续重新打开对话失去找回服务端任务的依据。
- 完成状态查询会把 Agent 结果（含 base64 图片和 Responses 输出）一次性放进同一个 JSON；最近记录的响应体达到约 1–2 MB。移动端恢复时应先查询轻量状态，再单独请求完成结果，降低状态轮询因大响应失败的概率。

### 本轮重构决定

- 服务端任务不设置浏览器侧绝对超时；客户端持续轮询，只有显式停止、服务端明确返回 `error` 或任务不存在才结束。
- 轮询网络错误、页面挂起后的恢复和响应体读取失败都采用退避重试，保留 `serverTaskId` 和 Agent round 的 `running` 状态。
- 状态接口默认只返回任务元数据；完成结果通过独立 `/result` 接口获取，前端拿到 `done` 后再读取一次结果。
- 页面 `pageshow`/`visibilitychange` 触发时立即接管所有带 `serverTaskId` 的 running round；已被旧版本错误标记为超时的轮次，仍保留的任务 ID也会被恢复。
- 为兼容旧页面，`GET /api-agent-tasks/:id` 默认仍可返回完整结果；新前端轮询时显式使用 `?meta=1`，完成后再访问 `/api-agent-tasks/:id/result`。
- 轮询重试只对网络错误、内部请求超时和 408/425/429/5xx 重试；服务端明确的 404、任务 `error` 和无效完成结果仍然会结束任务，避免把真正业务错误无限重试。
