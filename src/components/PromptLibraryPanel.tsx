...
interface PromptLibraryPanelProps {
  ...
  onCopyPrompt: (item: AwesomePromptCase) => void
...
function LibraryIcon({ type, className = 'h-4 w-4' }: { type: 'search' | 'refresh' | 'copy' | 'download' | 'external' | 'sparkles'; className?: string }) {
  const paths = {
...
function PromptCaseCard({
  ...
  return (
    <article className="group overflow-hidden rounded-xl border border-gray-200/80 bg-white transition duration-200 hover:border-violet-300 hover:shadow-md dark:border-white/[0.07] dark:bg-[#0d1015] dark:hover:border-violet-400/30 dark:hover:bg-[#111621]">
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100 dark:bg-[#171c27]">
        {imageFailed ? (
      ...
      <div className="p-2.5">
        ...
        <div className="mt-2.5 grid grid-cols-2 gap-1.5">
          <button type="button" onClick={onUsePrompt} className="flex h-11 items-center justify-center gap-1 rounded-lg bg-violet-600/90 px-1 text-[10px] font-semibold text-white transition hover:bg-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-400/60" title="将完整 Prompt 填入输入框">
            <LibraryIcon type="sparkles" className="h-3.5 w-3.5" />使用
          </button>
          <button type="button" onClick={onCopyPrompt} className="flex h-11 items-center justify-center gap-1 rounded-lg border border-gray-200 px-1 text-[10px] text-gray-600 transition hover:border-gray-300 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-400/60 dark:border-white/[0.08] dark:text-gray-400 dark:hover:border-white/[0.18] dark:hover:text-white" title="复制完整 Prompt">
            <LibraryIcon type="copy" className="h-3.5 w-3.5" />复制
          </button>
        </div>
...
export default function PromptLibraryPanel({ canImportImage, importingId, onCopyPrompt, onImportImage, onUsePrompt }: PromptLibraryPanelProps) {
